...
export const bishopMovement: MovementPattern = (pos, { board }) => {
  ...
  for (
    i = 1, j = 1;
    isWithinBounds({ row: pos.row + i, col: pos.col + j }, board.tokens);
    i++, j++
  ) {
...
export const rookMovement: MovementPattern = (pos, { board }) => {
  ...
  for (
    let i = 1;
    isWithinBounds({ row: pos.row + i, col: pos.col }, board.tokens);
    i++
  ) {
  ...
  for (
    let i = 1;
    isWithinBounds({ row: pos.row, col: pos.col + i }, board.tokens);
    i++
  ) {
...
export const getAllPossibleMoves = (
  ...
  for (let i = 0; i < rowCount; i++) {
    for (let j = 0; j < colCount; j++) {
      if (
        !G.board.tokens[i][j] ||
        G.board.tokens[i][j]?.playerID != ctx.currentPlayer
      ) {
        continue;
      }
      pos = { row: i, col: j };
...
const possibleMovesAtPos = (
  ...
  const reachablePositions: GridPosition[] = tileTemplate.movementPattern(
    gridPos,
    G,
  );
