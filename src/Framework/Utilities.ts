  ...
  for (let i = 0; i < rows; i++) {
    for (let j = 0; j < cols; j++) {
      grid[i][j] = {
        ...
        tileTypeName: TileTypeName.Blank,
      };
    }
  }
  return grid;
...
export const isMoveInOptions = (
  move: MoveDescription,
  options: MoveDescription[],
) => {
  return options.some((md) => compareMoveOptions(md, move));
};

export const compareMoveOptions = (
  move1: MoveDescription,
  move2: MoveDescription,
): boolean => {
  return (
    compareGridPositions(move1.from, move2.from) &&
    compareGridPositions(move1.to, move2.to)
  );
};

export const compareGridPositions = (
  pos1: GridPosition,
  pos2: GridPosition,
): boolean => {
...
export const setTileAtPosition = (
  G: GameState,
  pos: GridPosition,
  tile: Tile,
) => {
...
export const setTokenAtPosition = (
  G: GameState,
  pos: GridPosition,
  token: Token,
) => {
